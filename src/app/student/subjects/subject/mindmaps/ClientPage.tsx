...
const sortMindMaps = (items: MindMap[], modulesList: any[]) => {
  return [...items].sort((a, b) => {
    ...
    const getSubtopicInfo = (item: MindMap, mod: any) => {
...
export default function StudentMindMapsList() {
  ...
  const loadData = async () => {
    try {
      ...
      const sortedMaps = Array.isArray(mapsData) ? sortMindMaps(mapsData, modsData) : [];
        ...
        {mindmaps.length > 0 ? (
          ...
          >
            {mindmaps.map((map) => {
              ...
              const cardContent = (
                <Card className={`overflow-hidden transition-all duration-300 flex flex-col h-auto ${isPremiumTheme
          ...
          <div className={`text-center py-16 font-bold border border-dashed rounded-lg ${isPremiumTheme
            ...
            <p className="text-sm text-slate-500 font-normal mt-1">Your faculty hasn't uploaded any mind maps yet.</p>
          </div>
        )}
      </div>
    </div>

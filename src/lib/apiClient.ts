...
declare global {
  interface Window {
    ...
    _subjectsJsonPromise?: Promise<any>;
  }
}
  ...
  if (isDeployed && !isFaculty) {
    if (typeof window !== 'undefined') {
      ...
      if (action === 'getSubjects') {
        if (!window._subjectsJsonPromise) {
          window._subjectsJsonPromise = fetch(`${basePath}/subjects.json?v=${buildTime}`)
            .then(res => res.json())
            .catch(err => {
              console.error("Failed to load local subjects.json:", err);
              return null;
            });
        }
        try {
          const subjectsJson = await window._subjectsJsonPromise;
          if (subjectsJson) {
            return subjectsJson.getSubjects || [];
          }
        } catch (err) {
